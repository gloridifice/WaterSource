...
public class WaterLevelHUD extends AbstractGui {
    ...
    public void render(MatrixStack matrixStack, int screenWidth, int screenHeight, WaterLevelCapability.Data capData, double toughness) {
        ...
        for (int k6 = 0; k6 < 10; ++k6) {
            ...
            this.blit(matrixStack, OffsetX1, OffsetY1, 36 + texU2, texV, WIDTH, HEIGHT);

            if (k6 * 2 + 1 < waterLevel) {
                this.blit(matrixStack, OffsetX1, OffsetY1, texU1, texV, WIDTH, HEIGHT);
            }
            if (k6 * 2 + 1 == waterLevel) {
                this.blit(matrixStack, OffsetX1, OffsetY1, texU1 + 9, texV, WIDTH, HEIGHT);
            }

            //Water Saturation Level↓
            if (ConfigRegistry.OPEN_WATER_SATURATION_LEVEL.get()) {
                if (k6 * 2 + 1 < waterSaturationLevel) {
                    this.blit(matrixStack, OffsetX1, OffsetY1 - 1, texU1, texV + 9, 9, 9);
                    this.blit(matrixStack, OffsetX1, OffsetY1 + 1, texU1 + 9, texV + 9, 9, 9);
                }
                if (k6 * 2 + 1 == waterSaturationLevel) {
                    this.blit(matrixStack, OffsetX1, OffsetY1, texU1, texV + 9, 9, 9);
                }
            }
        }
        //test↓
