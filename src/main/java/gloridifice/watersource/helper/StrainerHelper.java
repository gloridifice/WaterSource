...
public class StrainerHelper {
    public static ItemStack getResultStack(ItemStack stack, StrainerBlock block) {
        ...
        if (!stack.isEmpty() && ModTags.Block.STRAINERS.contains(block) && block != BlockRegistry.BLOCK_DIRTY_STRAINER && FluidUtil.getFluidHandler(stack) != null) {
            FluidUtil.getFluidHandler(itemStack).ifPresent(data -> {
                WaterFilterRecipe recipe = WaterFilterRecipeManager.getRecipeFromInput(new ItemStack(Item.getItemFromBlock(block)), data.getFluidInTank(0).getFluid());
                if (recipe != null){
