...
public class StrainerHelper {
    public static ItemStack getResultStack(ItemStack stack, StrainerBlock block) {
        ...
        if (!stack.isEmpty() && BlockTags.getAllTags().getTag(new ResourceLocation(WaterSource.MODID, "strainer")).contains(block) && block != BlockRegistry.BLOCK_DIRTY_STRAINER && FluidUtil.getFluidHandler(stack) != null) {
