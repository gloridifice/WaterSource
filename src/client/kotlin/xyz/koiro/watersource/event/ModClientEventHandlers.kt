            ...
            if (!hydrationData.isDry()) {
                ...
                if (emptyCount <= 4) {
                    ...
                    for (i in 0..<satCount) {
                        ...
                        if (isEndAndHalf) {
                            DrawWaterBallUtils(newX, newY, isThirty, DrawWaterBallUtils.Part.SaturationDown, true).draw(context)
                        } else {
                            DrawWaterBallUtils(newX, newY, isThirty, DrawWaterBallUtils.Part.SaturationUp, true).draw(context)
                            DrawWaterBallUtils(newX, newY, isThirty, DrawWaterBallUtils.Part.SaturationDown, true).draw(context)
                        }
                    }
